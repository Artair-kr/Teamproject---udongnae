...
클라이언트로부터 이메일을 입력받아 해당하는 이메일에 4자리의 인증코드를 전송합니다. 이메일 전송이 성공적으로 종료되었으면 성공처리를 합니다.
...
***
### 이메일 인증 확인
email_auth 테이블에 존재하는 이메일과 인증번호를 확인합니다.
인증번호와 이메일이 맞다면 성공처리를 합니다.
인증에 실패한다면 AF, 데이터베이스 오류시 DBE 처리를 합니다.
- method : **POST**  
